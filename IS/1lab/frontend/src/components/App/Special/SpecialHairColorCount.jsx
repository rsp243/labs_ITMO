...
export default function SpecialHairColorCount({ getToken }) {
    ...
    const handleHairColorCount = _ => {
        ...
        .then(res => {
            ...
            for (let person in results) {
                if (results[person].hairColor === color) {
                    _count++
                }
            }
            
            msgs.current.show([
                { severity: 'success', life: 10000, summary: 'Success', detail: "Count of object with " + color + " hair color is " + _count, sticky: false, closable: true }
            ]);
        })
